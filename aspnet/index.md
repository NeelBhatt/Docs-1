---
uid: index
---
<a name=index></a>

  # ASP.NET Core Documentation

Attention: ASP.NET 5 has been renamed to ASP.NET Core 1.0. Read [more](https://blogs.msdn.microsoft.com/webdev/2016/02/01/an-update-on-asp-net-core-and-net-core/).

Note: This documentation is a work in progress. Topics marked with a 🔧 are placeholders that have not been written yet. You can track the status of these topics through our public documentation [issue tracker](https://github.com/aspnet/docs/issues). Learn how you can [contribute](https://github.com/aspnet/Docs/blob/master/CONTRIBUTING.md) on GitHub.

  ## Topics

* [Introduction](intro.md)

* [Getting Started](getting-started.md)

* [Release notes](https://github.com/aspnet/home/releases.md)

* [Tutorials](tutorials/index.md)

  * [Your First ASP.NET Core Application on a Mac Using Visual Studio Code](tutorials/your-first-mac-aspnet.md)

  * [Building Your First Web API with ASP.NET Core MVC and Visual Studio](tutorials/first-web-api.md)

  * [Deploy an ASP.NET Core web app to Azure using Visual Studio](tutorials/publish-to-azure-webapp-using-vs.md)

  * [Building your first ASP.NET Core MVC app with Visual Studio](tutorials/first-mvc-app/index.md)

    * [Getting started with ASP.NET Core MVC and Visual Studio](tutorials/first-mvc-app/start-mvc.md)

    * [Adding a controller](tutorials/first-mvc-app/adding-controller.md)

    * [Adding a view](tutorials/first-mvc-app/adding-view.md)

    * [Adding a model](tutorials/first-mvc-app/adding-model.md)

    * [Working with SQL Server LocalDB](tutorials/first-mvc-app/working-with-sql.md)

    * [Controller methods and views](tutorials/first-mvc-app/controller-methods-views.md)

    * [Adding Search](tutorials/first-mvc-app/search.md)

    * [Adding a New Field](tutorials/first-mvc-app/new-field.md)

    * [Adding Validation](tutorials/first-mvc-app/validation.md)

    * [Examining the Details and Delete methods](tutorials/first-mvc-app/details.md)

  * [ASP.NET Core on Nano Server](tutorials/nano-server.md)

  * [ASP.NET Core and Azure Service Fabric](https://azure.microsoft.com/en-us/documentation/articles/service-fabric-add-a-web-frontend/.md)

  * [🔧 Creating Backend Services for Native Mobile Applications](mobile/native-mobile-backend.md)

  * [Developing ASP.NET Core applications using dotnet watch](tutorials/dotnet-watch.md)

  * [ASP.NET Web API Help Pages using Swagger](tutorials/web-api-help-pages-using-swagger.md)

* [Fundamentals](fundamentals/index.md)

  * [Application Startup](fundamentals/startup.md)

  * [Middleware](fundamentals/middleware.md)

  * [Working with Static Files](fundamentals/static-files.md)

  * [Routing](fundamentals/routing.md)

  * [Error Handling](fundamentals/error-handling.md)

  * [Globalization and localization](fundamentals/localization.md)

  * [Configuration](fundamentals/configuration.md)

  * [Logging](fundamentals/logging.md)

  * [🔧 File Providers](fundamentals/file-providers.md)

  * [Dependency Injection](fundamentals/dependency-injection.md)

  * [Working with Multiple Environments](fundamentals/environments.md)

  * [Hosting](fundamentals/hosting.md)

  * [Managing Application State](fundamentals/app-state.md)

  * [Servers](fundamentals/servers.md)

  * [Request Features](fundamentals/request-features.md)

  * [Open Web Interface for .NET (OWIN)](fundamentals/owin.md)

  * [Choosing the Right .NET For You on the Server](fundamentals/choosing-the-right-dotnet.md)

* [MVC](mvc/index.md)

  * [Overview of ASP.NET Core MVC](mvc/overview.md)

  * [Models](mvc/models/index.md)

    * [Model Binding](mvc/models/model-binding.md)

    * [Model Validation](mvc/models/validation.md)

    * [Formatting Response Data](mvc/models/formatting.md)

    * [🔧 Custom Formatters](mvc/models/custom-formatters.md)

  * [Views](mvc/views/index.md)

    * [Views Overview](mvc/views/overview.md)

    * [Razor Syntax Reference](mvc/views/razor.md)

    * [Layout](mvc/views/layout.md)

    * [Working with Forms](mvc/views/working-with-forms.md)

    * [🔧 HTML Helpers](mvc/views/html-helpers.md)

    * [Tag Helpers](mvc/views/tag-helpers/index.md)

      * [Introduction to Tag Helpers](mvc/views/tag-helpers/intro.md)

      * [Working with Forms](mvc/views/working-with-forms.md)

      * [Authoring Tag Helpers](mvc/views/tag-helpers/authoring.md)

    * [Partial Views](mvc/views/partial.md)

    * [Dependency injection into views](mvc/views/dependency-injection.md)

    * [View Components](mvc/views/view-components.md)

    * [🔧 Creating a Custom View Engine](mvc/views/custom-view-engine.md)

  * [Controllers](mvc/controllers/index.md)

    * [Controllers, Actions, and Action Results](mvc/controllers/actions.md)

    * [🔧 Routing to Controller Actions](mvc/controllers/routing.md)

    * [Filters](mvc/controllers/filters.md)

    * [Dependency Injection and Controllers](mvc/controllers/dependency-injection.md)

    * [Testing Controller Logic](mvc/controllers/testing.md)

    * [Areas](mvc/controllers/areas.md)

    * [🔧 Working with the Application Model](mvc/controllers/application-model.md)

* [Testing](testing/index.md)

  * [Unit Testing](https://docs.microsoft.com/en-us/dotnet/articles/core/testing/unit-testing-with-dotnet-test.md)

  * [Integration Testing](testing/integration-testing.md)

  * [Testing Controller Logic](mvc/controllers/testing.md)

* [Working with Data](data/index.md)

  * [Getting Started with ASP.NET Core and Entity Framework 6](data/entity-framework-6.md)

  * [Getting Started With ASP.NET Core and Entity Framework Core](http://docs.efproject.net/en/latest/platforms/aspnetcore/index.html.md)

  * [Azure Storage](data/azure-storage/index.md)

    * [Adding Azure Storage by Using Visual Studio Connected Services](https://azure.microsoft.com/documentation/articles/vs-azure-tools-connected-services-storage/.md)

    * [Get Started with Azure Blob storage and Visual Studio Connected Services](https://azure.microsoft.com/documentation/articles/vs-storage-aspnet5-getting-started-blobs/.md)

    * [Get Started with Queue Storage and Visual Studio Connected Services](https://azure.microsoft.com/documentation/articles/vs-storage-aspnet5-getting-started-queues/.md)

    * [How to Get Started with Azure Table Storage and Visual Studio Connected Services](https://azure.microsoft.com/documentation/articles/vs-storage-aspnet5-getting-started-tables/.md)

* [Client-Side Development](client-side/index.md)

  * [Using Gulp](client-side/using-gulp.md)

  * [Using Grunt](client-side/using-grunt.md)

  * [Manage Client-Side Packages with Bower](client-side/bower.md)

  * [Building Beautiful, Responsive Sites with Bootstrap](client-side/bootstrap.md)

  * [Knockout.js MVVM Framework](client-side/knockout.md)

  * [Using Angular for Single Page Applications (SPAs)](client-side/angular.md)

  * [Styling Applications with Less, Sass, and Font Awesome](client-side/less-sass-fa.md)

  * [Bundling and Minification](client-side/bundling-and-minification.md)

  * [🔧 Working with a Content Delivery Network (CDN)](client-side/cdn.md)

  * [🔧 Responsive Design for the Mobile Web](client-side/responsive-design.md)

  * [TypeScript](https://www.typescriptlang.org/docs/handbook/asp-net-core.html.md)

  * [Building Projects with Yeoman](client-side/yeoman.md)

* [Mobile](mobile/index.md)

  * [🔧 Responsive Design for the Mobile Web](client-side/responsive-design.md)

  * [🔧 Creating Backend Services for Native Mobile Applications](mobile/native-mobile-backend.md)

* [Publishing and Deployment](publishing/index.md)

  * [Publishing to IIS](publishing/iis.md)

  * [Publishing to IIS with Web Deploy using Visual Studio](publishing/iis-with-msdeploy.md)

  * [How Web Publishing In Visual Studio Works](publishing/web-publishing-vs.md)

  * [Deploy an ASP.NET Core web app to Azure using Visual Studio](tutorials/publish-to-azure-webapp-using-vs.md)

  * [Publishing to an Azure Web App with Continuous Deployment](publishing/azure-continuous-deployment.md)

  * [🔧 Publishing to a Windows Virtual Machine on Azure](publishing/azure-windows-vm.md)

  * [Publish to a Docker Image](https://azure.microsoft.com/documentation/articles/vs-azure-tools-docker-hosting-web-apps-in-docker/.md)

  * [Publish to a Linux Production Environment](publishing/linuxproduction.md)

  * [Use VSTS to Build and Publish to an Azure Web App with Continuous Deployment](publishing/vsts-continuous-deployment.md)

* [Guidance for Hosting Providers](hosting/index.md)

  * [ASP.NET Core Module Configuration Reference](hosting/aspnet-core-module.md)

  * [Directory Structure](hosting/directory-structure.md)

  * [Application Pools](hosting/apppool.md)

  * [Servicing](hosting/servicing.md)

  * [Data Protection](hosting/dataprotection.md)

* [Security](security/index.md)

  * [Authentication](security/authentication/index.md)

    * [Introduction to Identity](security/authentication/identity.md)

    * [Enabling authentication using Facebook, Google and other external providers](security/authentication/sociallogins.md)

    * [Account Confirmation and Password Recovery](security/authentication/accconfirm.md)

    * [Two-factor authentication with SMS](security/authentication/2fa.md)

    * [🔧 Supporting Third Party Clients using OAuth 2.0](security/authentication/oauth2.md)

    * [Using Cookie Middleware without ASP.NET Core Identity](security/authentication/cookie.md)

    * [Azure Active Directory](security/authentication/azure-active-directory/index.md)

      * [Integrating Azure AD Into an ASP.NET Core Web App](https://azure.microsoft.com/documentation/samples/active-directory-dotnet-webapp-openidconnect-aspnetcore.md)

      * [Calling a ASP.NET Core Web API From a WPF Application Using Azure AD](https://azure.microsoft.com/documentation/samples/active-directory-dotnet-native-aspnetcore.md)

      * [Calling a Web API in an ASP.NET Core Web Application Using Azure AD](https://azure.microsoft.com/en-us/documentation/samples/active-directory-dotnet-webapp-webapi-openidconnect-aspnetcore/.md)

    * [Securing ASP.NET Core apps with IdentityServer4](https://identityserver4.readthedocs.io.md)

  * [Authorization](security/authorization/index.md)

    * [Introduction](security/authorization/introduction.md)

    * [Simple Authorization](security/authorization/simple.md)

    * [Role based Authorization](security/authorization/roles.md)

    * [Claims-Based Authorization](security/authorization/claims.md)

    * [Custom Policy-Based Authorization](security/authorization/policies.md)

    * [Dependency Injection in requirement handlers](security/authorization/dependencyinjection.md)

    * [Resource Based Authorization](security/authorization/resourcebased.md)

    * [View Based Authorization](security/authorization/views.md)

    * [Limiting identity by scheme](security/authorization/limitingidentitybyscheme.md)

    * [🔧 Authorization Filters](security/authorization/authorization-filters.md)

  * [Data Protection](security/data-protection/index.md)

    * [Introduction to Data Protection](security/data-protection/introduction.md)

    * [Getting Started with the Data Protection APIs](security/data-protection/using-data-protection.md)

    * [Consumer APIs](security/data-protection/consumer-apis/index.md)

      * [Consumer APIs Overview](security/data-protection/consumer-apis/overview.md)

      * [Purpose Strings](security/data-protection/consumer-apis/purpose-strings.md)

      * [Purpose hierarchy and multi-tenancy](security/data-protection/consumer-apis/purpose-strings-multitenancy.md)

      * [Password Hashing](security/data-protection/consumer-apis/password-hashing.md)

      * [Limiting the lifetime of protected payloads](security/data-protection/consumer-apis/limited-lifetime-payloads.md)

      * [Unprotecting payloads whose keys have been revoked](security/data-protection/consumer-apis/dangerous-unprotect.md)

    * [Configuration](security/data-protection/configuration/index.md)

      * [Configuring Data Protection](security/data-protection/configuration/overview.md)

      * [Default Settings](security/data-protection/configuration/default-settings.md)

      * [Machine Wide Policy](security/data-protection/configuration/machine-wide-policy.md)

      * [Non DI Aware Scenarios](security/data-protection/configuration/non-di-scenarios.md)

    * [Extensibility APIs](security/data-protection/extensibility/index.md)

      * [Core cryptography extensibility](security/data-protection/extensibility/core-crypto.md)

      * [Key management extensibility](security/data-protection/extensibility/key-management.md)

      * [Miscellaneous APIs](security/data-protection/extensibility/misc-apis.md)

    * [Implementation](security/data-protection/implementation/index.md)

      * [Authenticated encryption details.](security/data-protection/implementation/authenticated-encryption-details.md)

      * [Subkey Derivation and Authenticated Encryption](security/data-protection/implementation/subkeyderivation.md)

      * [Context headers](security/data-protection/implementation/context-headers.md)

      * [Key Management](security/data-protection/implementation/key-management.md)

      * [Key Storage Providers](security/data-protection/implementation/key-storage-providers.md)

      * [Key Encryption At Rest](security/data-protection/implementation/key-encryption-at-rest.md)

      * [Key Immutability and Changing Settings](security/data-protection/implementation/key-immutability.md)

      * [Key Storage Format](security/data-protection/implementation/key-storage-format.md)

      * [Ephemeral data protection providers](security/data-protection/implementation/key-storage-ephemeral.md)

    * [Compatibility](security/data-protection/compatibility/index.md)

      * [Sharing cookies between applications](security/data-protection/compatibility/cookie-sharing.md)

      * [Replacing <machineKey> in ASP.NET](security/data-protection/compatibility/replacing-machinekey.md)

  * [Safe storage of app secrets during development](security/app-secrets.md)

  * [🔧 Enforcing SSL](security/enforcing-ssl.md)

  * [🔧 Anti-Request Forgery](security/anti-request-forgery.md)

  * [🔧 Preventing Open Redirect Attacks](security/open-redirect.md)

  * [Preventing Cross-Site Scripting](security/cross-site-scripting.md)

  * [Enabling Cross-Origin Requests (CORS)](security/cors.md)

* [Performance](performance/index.md)

  * [🔧 Measuring Application Performance](performance/measuring.md)

  * [Caching](performance/caching/index.md)

    * [In Memory Caching](performance/caching/memory.md)

    * [Working with a Distributed Cache](performance/caching/distributed.md)

    * [Response Caching](performance/caching/response.md)

    * [🔧 Output Caching](performance/caching/output.md)

* [Migration](migration/index.md)

  * [Migrating From ASP.NET MVC to ASP.NET Core MVC](migration/mvc.md)

  * [Migrating Configuration](migration/configuration.md)

  * [Migrating Authentication and Identity](migration/identity.md)

  * [Migrating from ASP.NET Web API](migration/webapi.md)

  * [Migrating HTTP Modules to Middleware](migration/http-modules.md)

  * [Migrating from ASP.NET 5 RC1 to ASP.NET Core 1.0](migration/rc1-to-rtm.md)

  * [Migrating from ASP.NET Core RC2 to ASP.NET Core 1.0](migration/rc2-to-rtm.md)

* [API](https://docs.asp.net/projects/api.md)

* [Contribute](contribute/index.md)

  * [ASP.NET Contributing Guide](https://github.com/aspnet/Home/blob/dev/CONTRIBUTING.md.md)

  * [ASP.NET Docs Contributing Guide](https://github.com/aspnet/Docs/blob/master/CONTRIBUTING.md.md)

  * [ASP.NET Docs Style Guide](contribute/style-guide.md)

  ## Related Resources

* [.NET Core](https://microsoft.com/net/core)

* [Entity Framework Core](https://docs.efproject.net)

* [WebHooks](http://docs.asp.net/projects/aspnetwebhooks)

  ## Contribute

The documentation on this site is the handiwork of our many [contributors](https://github.com/aspnet/docs/contributors).

**We accept pull requests!** But you're more likely to have yours accepted if you follow these guidelines:

   1. Read [https://github.com/aspnet/Docs/blob/master/CONTRIBUTING.md](https://github.com/aspnet/Docs/blob/master/CONTRIBUTING.md)

   2. Follow the [ASP.NET Docs Style Guide](contribute/style-guide.md#style-guide.md)
